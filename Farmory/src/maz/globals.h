...
#ifdef DEBUG
#define MAZ_ASSERT(expression, format, ...) if(!(expression)) maz::Logger(__FILE__, __LINE__, format, "ASSERT: ", __VA_ARGS__); assert(expression);
#define MAZ_ERROR(format, ...) maz::Logger(__FILE__, __LINE__, format, "ERROR: ", __VA_ARGS__); assert(false);
#else
